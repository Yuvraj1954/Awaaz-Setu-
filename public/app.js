...
function setUiState(listening) {
    ...
    if (listening) { icon.style.display = 'none'; wave.style.display = 'flex'; container.classList.add('pulse-active'); }
    else { icon.style.display = 'block'; wave.style.display = 'none'; container.classList.remove('pulse-active'); }
}
...
function updateSidebarUI(items) {
    ...
    container.innerHTML = ""; 
    items.forEach(item => {
        const div = document.createElement('div'); div.style = "padding: 10px; font-size: 0.8rem; margin-top: 8px; cursor: pointer; color: #94a3b8; border-radius: 10px; background: rgba(255,255,255,0.05);";
...
if ('webkitSpeechRecognition' in window) {
    ...
    recognition.onstart = () => { isListening = true; setUiState(true); document.getElementById('mic-label').textContent = translations[currentLanguage].listening; };
    recognition.onresult = (e) => { document.getElementById('user-input').value = e.results[0][0].transcript; };
    recognition.onend = () => { isListening = false; setUiState(false); document.getElementById('mic-label').textContent = translations[currentLanguage].micLabel; if (document.getElementById('user-input').value) submitQuery(); };
}
